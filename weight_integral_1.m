function [it, error_it] = weight_integral_1(glz,Fn, Rm, deltam, gammas, errorb, H, q, s)
%
% [IT, ERROR_IT] = WEIGHT_INTEGRAL_1(FN, RM, DELTAM, GAMMAS, ERRORB, H, Q, S)
%
...
for k=1:n
    if gammas(k) == 1
        it(k)=0; error_it(k)= 0;%error_it2(k)= 0;
           ...
           eone = @(x)(x.*Phin(glz,q,H,x).*cos(gammas(k).*s(k).*x.^2.*Fn).^2 ...
                .*besselj(1,gammas(k).*Rm.*x).^2 ...
                .*(1/2-besselj(1,(1-gammas(k)).*deltam.*x)./((1-gammas(k)).*deltam.*x)));
            
        [it(k),error_it(k)]= quadgk(eone,0,Inf,'RelTol',0,'AbsTol',errorb,... 
